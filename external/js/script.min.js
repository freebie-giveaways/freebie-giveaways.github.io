...
POTENZA.masonry=function(){var $masonry=$('.masonry .masonry'),$itemElement='.masonry .masonry-item';if($masonry.exists()){$masonry.isotope({resizable:true,itemSelector:$itemElement,masonry:{gutterWidth:10}});}}
...
POTENZA.sidemenu=function(){var $menu_btn=$('.mobile-nav-button'),$overlay=$('.menu-overlay'),$menucls=$('.menu-close'),$slidemenu=$('.side-content');$menu_btn.on('click',function(){if($(".search").exists()){if($(".search").hasClass('is-visible')){return false;}}
toggleslidemenu();return false;});$overlay.on('click',function(){toggleslidemenu('close');return false;});$menucls.on('click',function(){toggleslidemenu('close');return false;});function toggleslidemenu(type){if(type=="close"){$slidemenu.removeClass('side-content-open');$overlay.removeClass('is-visible');}else{$slidemenu.addClass('side-content-open');$overlay.addClass('is-visible');}}}
POTENZA.search=function(){var $search_btn=$('.search-trigger'),$overlay=$('.search-overlay'),$search_area=$('.search');$search_btn.on('click',function(){var $type='';($search_area.hasClass('is-visible'))?$type='close':$type='';toggleSearch($type);return false;});$search_btn.on('click',function(){if($(".side-content-open").exists()){toggleSearch('close');return false;}});$overlay.on('click',function(){toggleSearch('close');return false;});function toggleSearch(type){if(type=="close"){$search_area.removeClass('is-visible');$search_btn.removeClass('search-is-visible');$overlay.removeClass('is-visible');$body.css({"overflow":"visible"});}else{$search_btn.addClass('search-is-visible');$search_area.addClass('is-visible');$overlay.addClass('is-visible');$search_area.find('input[type="search"]').focus();$body.css({"overflow":"hidden"});}}}
...
var progressBarRun=function(){$elem.animate({'width':percent+'%'},'easeInOutCirc').addClass('progress-animated');$elem.delay(delay).append('<span class="progress-type animated fadeIn">'+type+'</span><span class="progress-number animated fadeIn">'+percent+'</span>');};$(elem).appear(function(){setTimeout(function(){progressBarRun();},delay);});});}};POTENZA.scrolltotop=function(){var $scrolltop=$('.back-to-top');$scrolltop.on('click',function(){$('html,body').animate({scrollTop:0},800);$(this).addClass("back-run");setTimeout(function(){$scrolltop.removeClass('back-run');},1000);return false;});$window.on('scroll',function(){if($window.scrollTop()>=1000){$scrolltop.addClass("show");$scrolltop.addClass("back-down");}else{$scrolltop.removeClass("show");setTimeout(function(){$scrolltop.removeClass('back-down');},300);}});$(".back-to-top img").mouseover(function(){$(this).attr('src',$(this).data("hover"));}).mouseout(function(){$(this).attr('src',$(this).data("src"));});}
POTENZA.stickyFooter=function(){var $footerfixed=$("#footer-fixed");if($footerfixed.exists()){var footerheight=$('#footer-fixed').height()+70;document.getElementById("main").style.marginBottom=footerheight+"px";}};POTENZA.slickslider=function(){if($(".slider-slick").exists()){$('.slider-for').slick({slidesToShow:1,slidesToScroll:1,arrows:true,asNavFor:'.slider-nav'});$('.slider-nav').slick({slidesToShow:3,slidesToScroll:1,asNavFor:'.slider-for',dots:false,centerMode:true,focusOnSelect:true});}}
...
$window.load(function(){POTENZA.preloader(),POTENZA.Isotope(),POTENZA.progressBar(),POTENZA.masonry();});$document.ready(function(){POTENZA.megaMenu(),POTENZA.counters(),POTENZA.carousel(),POTENZA.tabs(),POTENZA.accordion(),POTENZA.countdownTimer(),POTENZA.search(),POTENZA.sidemenu(),POTENZA.scrolltotop(),POTENZA.mediaPopups(),POTENZA.stickyFooter(),POTENZA.slickslider(),POTENZA.Contactform();});})(jQuery);/*! Magnific Popup - v1.1.0 - 2016-02-20
